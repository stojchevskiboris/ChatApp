...
import { RecipientViewModel } from '../../../../models/recipient-view-model';
...
@Component({
  selector: 'app-recipients-registry',
  templateUrl: './recipients-registry.component.html',
  styleUrl: './recipients-registry.component.css'
})
export class RecipientsRegistryComponent implements OnInit {
  searchModel: RecipientSearchModel = {
    page: 0,
    size: 10,
    sortColumn: 'id',
    sortDirection: 'ASC'
  ...
  isAsc: boolean = true;
  pagedResult: PagedResultModel<RecipientViewModel>;
  ...
  search(): void {
    this.loading = true;
    this.adminService.searchRecipients(this.searchModel).subscribe({
      next: (result) => {
        this.loading = false;
  ...
  deleteRecipient(id: number): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!!result) {
        this.loading = true;
        this.adminService.deleteRecipient(id).subscribe({
          next: () => {
            this.loading = false;
            this.toastr.info('Recipient deleted successfully');
            this.search();
          },
          error: () => {
            this.loading = false;
            this.toastr.error('Failed to delete recipient');
          }
        });
      }
    });
