...
import { AdminMediaViewModel } from '../../../../models/admin-media-view-model';
...
@Component({
  selector: 'app-media-registry',
  templateUrl: './media-registry.component.html',
  styleUrl: './media-registry.component.css'
})
export class MediaRegistryComponent implements OnInit {
  searchModel: MediaSearchModel = {
    page: 0,
    size: 10,
    sortColumn: 'id',
    sortDirection: 'ASC'
  ...
  isAsc: boolean = true;
  pagedResult: PagedResultModel<AdminMediaViewModel>;
  ...
  search(): void {
    this.loading = true;
    this.adminService.searchMedia(this.searchModel).subscribe({
      next: (result) => {
        this.loading = false;
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!!result) {
        ...
        this.adminService.deleteMedia(id).subscribe({
          next: () => {
            this.loading = false;
            this.toastr.info('Media deleted successfully');
            this.search();
          },
          error: () => {
            this.loading = false;
